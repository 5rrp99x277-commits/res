...
This package fixes the GitHub Actions CMake parse error from the previous v4.15 archive.

Expected UF2 after GitHub Actions build:

```text
cyw4373_emulator_nohub_checked_v4_15.uf2
```

# v4.16 explicit USB connect test

This version is based on v4.15 and keeps:

```text
WL_REG_ON delay = 200 ms
```
...
A new UART line is printed:

```text
USB connect requested
```

Reason:

The plotter log reached:
...
This build tests whether the Pico/TinyUSB stack needs an explicit USB device connect/pull-up command.
...
# v4.17 runtime DL fix

Fixes this runtime/postboot UART event:

```text
USB mounted as runtime 04b4:0bdc
RUNTIME unhandled setup bm=0xC1 bReq=0x05 len=24
```

`bm=0xC1 bReq=0x05 len=24` is `DL_GETVER`.

Runtime/postboot now handles:

```text
DL_GETVER   -> chip = 0xA123
DL_GETSTATE -> state = DL_RUNNABLE
DL_RESETCFG -> ACK
```

Expected UART after runtime mount:

```text
USB mounted as runtime 04b4:0bdc
RUNTIME/POSTBOOT DL_GETVER -> chip=0xA123
```
